...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($destination) || empty($cruise_details) || empty($name) || empty($contact_number) || empty($department)) {
        ...
        $sql = "UPDATE cruise_details SET destination = ?, cruise_details = ?, name = ?, contact_number = ?, department = ?, email = ?, adult_price = ?, kids_price = ?, status = ? WHERE id = ?";
        
        if ($stmt = mysqli_prepare($conn, $sql)) {
            mysqli_stmt_bind_param($stmt, "ssssssdds", $destination, $cruise_details, $name, $contact_number, $department, $email, $adult_price, $kids_price, $status, $id);
            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <input type="email" name="email" class="form-control" value="<?php echo htmlspecialchars($cruise_detail['email']); ?>">
                    </div>
                </div>
            </div>
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <select name="status" class="form-control">
                            <option value="Active" <?php echo ($cruise_detail['status'] == 'Active') ? 'selected' : ''; ?>>Active</option>
                            <option value="Inactive" <?php echo ($cruise_detail['status'] == 'Inactive') ? 'selected' : ''; ?>>Inactive</option>
                        </select>
                    </div>
                </div>
            </div>
