...
if(!hasPrivilege('no_response_enquiries')) {
...
// Build the base SQL query with filters - Include multiple status types
$base_sql = "SELECT e.*, u.full_name as attended_by_name, d.name as department_name, 
        ...
        WHERE ls.name IN ('" . implode("', '", $follow_up_statuses) . "')";
...
$count_sql = "SELECT COUNT(DISTINCT e.id) 
        ...
        LEFT JOIN ad_campaigns ac ON e.ad_campaign_id = ac.id 
        JOIN lead_status ls ON e.status_id = ls.id 
        WHERE ls.name IN ( 'Discussion on going', 'No incoming call', 'Package shared', 'Postponed', 'Rescheduled request')";
...
<div class="card-box mb-30">
    <div class="pd-20">
        <h4 class="text-blue h4">Follow up Enquiries Filters</h4>
    </div>
...
<!-- No Response Enquiries Table -->
<div class="card-box mb-30">
    <div class="pd-20">
        <h4 class="text-blue h4">Follow up Enquiries - All Status Types (<?php echo $total_records; ?> total)</h4>
    </div>
    <div class="pb-20">
        <table class="data-table table stripe hover nowrap">
            ...
            <tbody>
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="11" class="text-center">No follow-up enquiries found</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
