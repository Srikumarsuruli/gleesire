...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($package_name) || empty($package_price) || empty($department_id) || empty($status)) {
        ...
        $sql = "INSERT INTO packages (package_name, package_price, department_id, status, created_at) VALUES (?, ?, ?, ?, NOW())";
        
        if ($stmt = mysqli_prepare($conn, $sql)) {
            mysqli_stmt_bind_param($stmt, "ssis", $package_name, $package_price, $department_id, $status);
...
<div class="pd-20 card-box mb-30">
    <div class="clearfix">
        <div class="pull-left">
            <h4 class="text-blue h4">Add New Package</h4>
        </div>
