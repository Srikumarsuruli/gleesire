...
while($current_date < $end_date_obj) {
    ...
    foreach($departments as $dept) {
        $report_data[$date_str][$dept['id']] = 0;
    }
    
    $current_date->modify('+1 day');
}

// Get enquiry counts by date and department
$sql = "SELECT DATE(received_datetime) as date, department_id, COUNT(*) as count 
        FROM enquiries 
        WHERE DATE(received_datetime) BETWEEN ? AND ?
        GROUP BY DATE(received_datetime), department_id
...
while($row = mysqli_fetch_assoc($result)) {
    ...
    if(isset($report_data[$date][$dept_id])) {
...
<div class="card-box mb-30">
    <div class="pd-20">
        <h4 class="text-blue h4">Department Wise Report</h4>
    </div>
    <div class="pb-20">
            ...
            <table class="table table-bordered" id="report-table">
                <thead>
                    <tr>
                        ...
                        <th><?php echo htmlspecialchars($dept['name']); ?></th>
                        <?php endforeach; ?>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <?php foreach($departments as $dept): ?>
                        <td><?php echo $dept_counts[$dept['id']]; ?></td>
                        <?php endforeach; ?>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
...
<script>
    document.getElementById('export-excel').addEventListener('click', function() {
        ...
        var filename = 'Department_Report_' + startDate + '_to_' + endDate + '.xlsx';
