...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        ...
        // Check if agent_package_data column exists, if not add it
        $check_column_sql = "SHOW COLUMNS FROM tour_costings LIKE 'agent_package_data'";
        $column_exists = mysqli_query($conn, $check_column_sql);
        
        if (mysqli_num_rows($column_exists) == 0) {
            // Column doesn't exist, add it
            $add_column_sql = "ALTER TABLE tour_costings ADD COLUMN agent_package_data TEXT AFTER extras_data";
            if (!mysqli_query($conn, $add_column_sql)) {
                throw new Exception("Failed to add agent_package_data column: " . mysqli_error($conn));
            }
        }
        ...
        // Initialize payment_receipt variable
        ...
        $insert_sql = "INSERT INTO tour_costings (
            ...
            tour_package, currency, nationality, selected_services, visa_data,
            accommodation_data, transportation_data, cruise_data, extras_data, agent_package_data,
            payment_data, total_expense, markup_percentage, markup_amount,
            tax_percentage, tax_amount, package_cost, currency_rate, converted_amount
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        
        $stmt = mysqli_prepare($conn, $insert_sql);
        // Count parameters: i(1) + s(15) + d(8) = 24 parameters
        mysqli_stmt_bind_param($stmt, "isssssssssssssssdddddddd",
            $enquiry_id, $cost_sheet_number, $guest_name, $guest_address, $whatsapp_number,
            $tour_package, $currency, $nationality, $selected_services, $visa_data,
            $accommodation_data, $transportation_data, $cruise_data, $extras_data, $agent_package_data,
            $payment_data, $total_expense, $markup_percentage, $markup_amount,
        ...
        if ($is_editing) {
            ...
            $update_sql = "UPDATE tour_costings SET 
                ...
                tour_package = ?, currency = ?, nationality = ?, selected_services = ?,
                visa_data = ?, accommodation_data = ?, transportation_data = ?,
                cruise_data = ?, extras_data = ?, agent_package_data = ?, payment_data = ?,
                total_expense = ?, markup_percentage = ?, markup_amount = ?,
                tax_percentage = ?, tax_amount = ?, package_cost = ?,
                currency_rate = ?, converted_amount = ?, updated_at = CURRENT_TIMESTAMP
                WHERE enquiry_id = ?";
            
            $update_stmt = mysqli_prepare($conn, $update_sql);
            // Count parameters: s(15) + d(8) + i(1) = 24 parameters
            mysqli_stmt_bind_param($update_stmt, "sssssssssssssssdddddddi",
                $guest_name, $guest_address, $whatsapp_number, $tour_package, $currency,
                $nationality, $selected_services, $visa_data, $accommodation_data,
                $transportation_data, $cruise_data, $extras_data, $agent_package_data, $payment_data,
                $total_expense, $markup_percentage, $markup_amount, $tax_percentage,
                $tax_amount, $package_cost, $currency_rate, $converted_amount, $enquiry_id
            );
        ...
        if (!empty($payment_date) && !empty($payment_bank) && $payment_amount > 0) {
            ...
            if (isset($_FILES['payment_receipt']) && $_FILES['payment_receipt']['error'] == 0) {
                ...
                if (move_uploaded_file($_FILES['payment_receipt']['tmp_name'], $target_file)) {
                    ...
                    // Also update the payment_data JSON to include the receipt path
                    $payment_data_array = json_decode($payment_data, true);
                    $payment_data_array['receipt'] = $target_file;
                    $payment_data = json_encode($payment_data_array);
                }
            }
