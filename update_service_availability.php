...
if(!isset($_SESSION["id"])) {
    error_log("Session not set, redirecting");
    header("location: " . $_POST['callback'] . "php");
    exit;
}

if(
    $_SERVER["REQUEST_METHOD"] == "POST"  && isset($_POST['service'])  && isset($_POST['callback'])  && isset($_POST['id'])  && isset($_POST['availability'])  && isset($_POST['idx'])
) {
    ...
    if($enquiry_id > 0 && $service) {
        
        $sql = 'UPDATE tour_costings SET ' . $service . '= JSON_SET(' . $service .', ?, ?), updated_at = NOW() WHERE id = ?;';
...
header("location:" . $_POST['callback'] . ".php?updated=" . time());
exit;
?>