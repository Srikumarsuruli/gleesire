...
if(!isset($_SESSION["id"])) {
    error_log("Session not set, redirecting to hotel_cruise_details.php");
...
if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['id']) && isset($_POST['availability']) && isset($_POST['idx'])) {
    ...
    if($enquiry_id > 0) {
        ...
        SET cruise_data = JSON_SET(cruise_data, ?, ?),
            updated_at = NOW()
        WHERE id = ?;";
...
header("location: hotel_cruise_details.php?updated=" . time());
exit;
?>